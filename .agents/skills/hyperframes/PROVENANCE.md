...
The `gsap` upstream skill is NOT re-vendored — Montagent already ships its
own GSAP skill family under `.agents/skills/gsap*/`.

## Local edits

Any divergence from upstream is noted at the top of the edited file as an
HTML comment starting with `Montagent-local`. Current edits:
...
- `hyperframes-cli/SKILL.md` — added `validate` to the command list and a
  ...
  Montagent's HyperFrames runtime path relies on `hyperframes validate` as
...
# From Montagent
cd C:/Users/ishan/Documents/Montagent
...
1. Montagent contributors may not have the HyperFrames monorepo on disk.
2. Skills must be readable from the Montagent tree for agent discovery.
