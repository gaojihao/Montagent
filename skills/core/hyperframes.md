...
This is the **Montagent-specific** guide to HyperFrames. It explains when
Montagent pipelines should choose HyperFrames over Remotion, how Montagent
...
## When Montagent should pick HyperFrames (vs Remotion vs FFmpeg)

Montagent separates two concepts:
...
| Synthetic UI / fake terminal / fake browser demo | Either — depends on existing coverage | Montagent already ships Remotion `TerminalScene` (see `synthetic-screen-recording` Layer 3). For UI chrome beyond terminal, HyperFrames HTML is easier. |
...
Montagent artifacts into HyperFrames project files:

| Montagent artifact field | HyperFrames target |
|---|---|
...
`STORYBOARD.md` as step-by-step workspace files. Montagent does **not**
...
Montagent playbooks currently translate into Remotion `themeConfig`
...
- ❌ Running `hyperframes init` from the Montagent orchestrator. `init`
