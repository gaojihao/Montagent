...
Layer 2: skills/                          "How Montagent uses these tools"
...
3. Layer 2 skills (this directory) teach the agent Montagent-specific conventions
...
| **Image Generation** | `flux-best-practices`, `bfl-api`, `grok-media` | `black-forest-labs/skills`, local Montagent skill |
...
| **AI Video (Premium)** | `seedance-2-0` — preferred premium default (cinematic, trailer, multi-shot, lip-sync, synced audio); accessed via `seedance_video` (fal.ai) or `heygen_video` Avatar Shots | Local Montagent skill |
