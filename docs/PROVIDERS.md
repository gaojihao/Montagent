# Montagent Provider Guide

Everything you need to know about every provider in Montagent — setup instructions, pricing, free tiers, and what each unlocks.
...
Montagent now uses those published rates in the Grok tool estimators.
...
Montagent uses the new-console API key flow:
...
Doubao Speech 2.0 is billed by character package or usage in Volcengine. Montagent estimates cost from text length and prefers provider-returned usage metadata when available.
...
The CLI is consumed as `npx hyperframes`. Do not use `npx @hyperframes/cli`; that package name is not the Montagent runtime path.
...
HyperFrames workspaces live under `projects/<project-name>/hyperframes/`. Final videos still follow the normal Montagent convention: `projects/<project-name>/renders/final.mp4`.
